...
export default function CreateEvent() {
    ...
    const getEventCategories = async () => {
        try {
            const res = await axios.get("http://localhost:5000/events/categories");
            const res2 = await axios.get("http://localhost:5000/events/cities");
    ...
    const onCreate = async () => {
        try {
            ...
            const eventDetails = {
                ...
                cityId: inputCity.current.value,
                dateTimeStart: `${inputDateStart.current.value} ${inputTimeStart.current.value}`,
                dateTimeEnd: `${inputDateEnd.current.value} ${inputTimeEnd.current.value}`,
                // dateTimeEnd: [inputDateEnd.current.value, inputTimeEnd.current.value],
    ...
    const tokenToId = async () => {
        try {
            const token = localStorage.getItem("idLogin");
            const { data } = await axios.get(`http://localhost:5000/users/tokentoid/${token}`)
